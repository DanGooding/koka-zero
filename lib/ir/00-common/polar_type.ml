...
let rec variables t =
  ...
  | List t | Option t -> variables t
  | Tuple ts -> Type.Variable.Set.union_list (List.map ts ~f:variables)
...
let rec simplify t =
  ...
  | Option t -> Option (simplify t)
  | Tuple ts -> Tuple (List.map ts ~f:simplify)
